...
- **項目名:** Temperature
- **説明:**
  AIの応答の「創造性」や「多様性」を調整するパラメータです。0.0から2.0の範囲で設定します。
  - **低い値（0に近い）:** より決まった、予測可能な応答になります。
  - **高い値（2に近い）:** より独創的で、多様な応答になります。あるいは壊れた応答になります。

#### 最大トークン数 (`MaxTokens`)
- **項目名:** 最大トークン数
- **説明:**
  AIが一度の応答で生成する文章の最大長の制限です。基本的に変更する必要はありません。
