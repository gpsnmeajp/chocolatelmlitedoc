...
システムプロンプトです。  
アシスタントの役割や振る舞いを指定します。  
この指示は全体に強力かつ根本的に作用しますが、後からの変更には即効性が低いことが多いです。

### ポストプロンプト
ユーザー発言の後に指示を埋め込みます。この指示は即効性を持ち強力に作用します。  
ポストプロンプトは会話履歴に残らず、最新の生成処理時に動的に適用されます。  
この機能が有効の間、最新のユーザー発言は`<user>`タグで囲まれ、次に`<system>`タグで囲まれたポストプロンプトが続きます。

例: ユーザーが`こんにちは、元気？`と発言し、ポストプロンプトに`必ず丁寧な口調で答えること。`と設定していた場合、AIには以下のように渡されます。

```
<user>こんにちは、元気？</user>
<system>必ず丁寧な口調で答えること。</system>
```
...
### キーワードナレッジ
ユーザー発言に含まれるキーワードに応じて、`<knowledge>`タグで補足情報を追加し、AIに渡します。  

補足情報は会話履歴に残らず、最新の生成処理時に動的に適用されるため、即効性があります。  
例えば、「自宅」と言うキーワードに、「ユーザーの住所は東京都」というナレッジを紐付けると、ユーザーが「自宅の天気は？」と尋ねた際に、AIは住所を参照できます。  

なお、キーワードは完全一致です。カンマ区切りで複数指定することも出来ます。  

いわゆる簡易的なRAGのように使うことが出来ます。  
キャラクター設定や、世界観設定など、細かい設定を思い出させるような用途にも使えます。

例: ユーザー入力で`自宅の天気を教えて`と言う発言があり、`自宅`や`天気`にキーワードを設定していた場合、以下のようにAIに渡されます。
...
```
knowledge: ユーザー発言に含まれるキーワードに応じて渡される補足情報です。
```


### メモリ一覧

メモリはペルソナごとに保存され、システムプロンプトの一部として使用されます。  
AIから編集可能な動的なシステムプロンプトとして機能します。  
一覧で現在の内容を確認できます。編集するにはAIに依頼するか、 memory.yaml を直接編集してください。  
